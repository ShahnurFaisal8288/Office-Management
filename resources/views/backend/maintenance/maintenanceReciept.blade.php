...
<body style="border: 2px solid black; margin:40px 60px 40px 60px;">
    <div style="margin:40px 60px">
        ...
        <div style="width:100%;display:inline-block;text-align:center;border: 1px dashed black;">
            <h1 style="font-size:16px;">Maintenance Money Reciept</h1>
        </div>
        <div style="width:100%;display:inline-block;">
            <div style="float: left;">
                <h4>Client Name: {{ $maintenancePay->customer->name }}</h4>
                <h4>TK: {{ $maintenancePay->maintenancePay->amount }} </h4>
                <h4>Payment Method: {{ $maintenancePay->maintenancePay->payment_type }}</h4>
            </div>
            <div style="float: right;">
                <h4>Date: {{ $maintenancePay->created_at->format('d-M-y') }}</h4>
                <h4>Branch Name: {{ $maintenancePay->branch_name }}</h4>
                <h4>Bank Name: {{ $maintenancePay->bank_name }}</h4>
            </div>
        </div>
