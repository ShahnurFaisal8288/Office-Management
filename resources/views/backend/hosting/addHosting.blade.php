                    ...
                    <div class="table-responsive">
                        <table class="table table-hover table-borderd" id="example">
                            ...
                            @foreach($hosting as $value)
                                ...
                                <tr>
                                    <td>{{ $i++ }}</td>
                                    <td>{{ $value->hostingName }}</td>
                                    <td>{{ $value->credentials }}</td>
                                    <td>{{ $value->startDate }}</td>
                                    <td @if($isCloseToEnd) style="background-color: red;color:aliceblue;" @endif>{{ $value->endDate }}</td>
                                    <td>
                                        <div class="action-buttons">
                                            <a class="btn btn-dark" href="{{ route('hosting.edit', $value->id) }}"><i class="fas fa-edit"></i></a>
                                            <form action="{{ route('hosting.destroy', $value->id) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Add Hosting</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="form-group my-2">
                        <label for="hostingName">Hosting Name</label>
                        <input type="text" class="form-control" name="hostingName" id="hostingName">
                    </div>
                    <div class="form-group my-2">
