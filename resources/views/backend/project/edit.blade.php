            ...
            <div class="card">
                <div class="d-flex align-items-end row">
                    <div class="col-sm-12">
                        <div class="card-body">
                            @include('error')
                            <form method="post" action="{{route('project.update',$projectId->id)}}">
                                @csrf
                                @method('PUT')
                                <div class="row">
                                    <div class="col-xxl">
                                            ...
                                            <div class="card-body">
                                                ...
                                                <div class="row my-3 d-flex">
                                                    <div class="col-lg-4">
                                                        ...
                                                        <select name="projectCreate_id" id="project_id" class="form-control my-2 select2">
                                                            ...
                                                            <option value="{{$item->id}}" {{ $projectId->projectCreate_id == $item->id ? 'selected' : '' }}>{{ $item->project_name }}</option>
                                                            @endforeach
                                                        </select>
                                                    </div>
                                                </div>
                                                <div class="show_transaction">
                                                    <div class="row item-row">
                                                        @php
                                                        $proDetails = \App\Models\ProjectDetails::where('project_create_id', $projectId->projectCreate_id)->get();
                                                    @endphp
                                                    @foreach($proDetails as $index => $item)
                                                        <div class="col-md-2 mb-3">
                                                            <label class="form-label">Module</label>
                                                            <select class="form-control module-select select2" name="project_module_id[]">
                                                                <option value="">Select</option>
                                                                @foreach($projectModule as $module)
                                                                    <option value="{{ $module->id }}" {{ $module->id == $item->project_module_id ? 'selected' : '' }}>
                                                                        {{ $module->module_name }}
                                                                    </option>
                                                                @endforeach
                                                            </select>
                                                        ...
                                                        <div class="col-md-3 mb-3">
                                                            ...
                                                            <select class="form-control features-select select2" name="features[{{ $index }}][]" multiple>
                                                                ...
                                                                @endforeach
                                                                <!-- Optionally, add other features to select -->
                                                            </select>
                                                        </div>
                                                        <div class="col-md-3 mb-3">
                                                        ...
                                                        <div class="col-md-1 mb-3">
                                                            ...
                                                            <input class="form-control" type="text" name="hours[]" value="{{ $item->hours }}">
                                                        </div>
                                                        <div class="col-md-2 mb-3">
                                                            ...
                                                            <select class="form-control select2" name="employee_id[{{ $index }}][]" multiple>
                                                                <option value="">Select</option>
                                                                @php
                                                                    $selectedEmployees = json_decode($item->employee_id, true) ?? [];
                                                                @endphp
                                                                @foreach($employee as $value)
                                                                    <option value="{{ $value->id }}" {{ in_array($value->id, $selectedEmployees) ? 'selected' : '' }}>
                                                                        {{ $value->name }}
                                                                    </option>
                                                                @endforeach
                                                            </select>
